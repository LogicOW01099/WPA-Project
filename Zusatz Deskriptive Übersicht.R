...
# Grundlage sind alle Zensus Dateien mit auskommentierter letzter Zeile
...
Zahlen_gender = bind_rows(Zahlen_gender2) %>% 
...
Zahlen_alter = bind_rows(Zahlen_alter2) %>% 
  group_by(Alter_7) %>% 
...
# Violin Plot
ggplot(age2, aes(x = "", y = V_ALTER)) +
  geom_violin(fill = "#00235A", alpha = 0.6) + # TUD BLUE
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) + # boxplot in der mitten
  labs(x = NULL, y = "Alter", title = "Altersverteilung") +
  theme_minimal()+
  coord_flip()
...
Zahlen_einkommen = bind_rows(Zahlen_einkommen2) %>% 
  group_by(Eink_5) %>% 
